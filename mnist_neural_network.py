...
def one_hot(Y):
    one_hot_Y = np.zeros((Y.size,
                          Y.max() + 1))  # Initialize an array with dimensions of Amount of Total Labels by Amount of Distinct Labels (10 digits in this case)
    one_hot_Y[np.arange(
        Y.size), Y] = 1  # For each row in the zeroed array, set the value at the corresponding digit as an index to 1
...
def forward_propagation(X, parameters):
    ...
    weights_hidden, biases_hidden, weights_output, biases_output = parameters['weights_hidden'], parameters[
        'biases_hidden'], parameters['weights_output'], parameters['biases_output']
...
def backward_propagation(parameters, outputs, X, Y):
    ...
    gradients = {"dweights_hidden": dweights_hidden, "dbiases_hidden": dbiases_hidden,
                 "dweights_output": dweights_output, "dbiases_output": dbiases_output}
...
def gradient_descent(parameters, gradients):
    ...
    parameters = {"weights_hidden": weights_hidden, "biases_hidden": biases_hidden, "weights_output": weights_output,
                  "biases_output": biases_output}
...
for i in range(0, int(tests)):
    ...
    current_image = current_image.reshape(
        (28, 28)) * 255  # Convert the normalized digit pixels back to a plottable image
    plt.gray()
...
correct_predictions = np.sum(
    predictions == Y_test[0])  # Correct predictions where the predicted digit equals the actual digit
...
print('Accuracy: ' + str(
    np.round((correct_predictions / total_digits) * 100, 1)) + '%')  # Calculate accuracy of the model